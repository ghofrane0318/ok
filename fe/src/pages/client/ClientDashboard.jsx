...
// Panier pré-rempli avec des produits de démonstration
const initialCart = [
  {
    _id: '1',
    nom: 'Essence Sans Plomb',
...
function ClientDashboard() {
  ...
  const [cart, setCart] = useState(initialCart); // Panier pré-rempli
  ...
  const addToCart = () => {
    ...
    addToast(`${selectedQuantity} ${selectedProduct.uniteMesure} de ${selectedProduct.nom} ajouté au panier`, 'success');
  ...
  const factures = useMemo(() => {
    return commandes
      ...
      .map((cmd, index) => ({
        ...
        dateEcheance: new Date(new Date(cmd.dateCreation).setDate(new Date(cmd.dateCreation).getDate() + 15)).toISOString(),
        montant: cmd.montantTotal,
        statutPaiement: cmd.statut === 'Livrée' ? 'Payée' : (cmd.statut === 'Validée' ? 'En attente' : 'Impayée'),
  ...
  const stats = useMemo(() => {
    ...
    const facturesStats = {
      ...
      payees: factures.filter(f => f.statutPaiement === 'Payée').length,
      impayees: factures.filter(f => f.statutPaiement === 'Impayée').length,
      montantTotal: factures.reduce((sum, f) => sum + f.montant, 0),
      montantRecouvre: factures.filter(f => f.statutPaiement === 'Payée').reduce((sum, f) => sum + f.montant, 0)
    };
    ...
    const livraisonsStats = {
      ...
      domicile: commandes.filter(c => c.typeLivraison === 'domicile' && c.statut !== 'Refusée').length,
      station: commandes.filter(c => c.typeLivraison === 'station' && c.statut !== 'Refusée').length,
  ...
  const passerCommande = useCallback(() => {
    ...
    const nouvelleCommande = {
      ...
      numeroCommande: `CMD-${now.getFullYear()}-${String(commandes.length + 1).padStart(4, '0')}`,
    ...
    <div className="client-dashboard petroleum-dashboard">
        ...
        <div className="modal-overlay" onClick={() => setShowQuantityModal(false)}>
          <div className="quantity-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-buttons">
              ...
              <button className="btn-confirm" onClick={addToCart}>Ajouter au panier</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="dashboard-header petroleum-header">
        <div className="header-content">
          <div className="logo-section">
            ...
            <h1>Station Pétrolière El Afia</h1>
          </div>
          <p>Bienvenue, {user.nom || 'Client'} ! Tableau de bord client</p>
        </div>
      ...
      {showStats && (
        <div className="stats-section">
          <h2 className="section-title">📊 Tableau de bord client</h2>
          ...
          <div className="stats-summary-cards">
            ...
            <div className="stat-card factures-card">
              ...
              <div className="stat-content">
                ...
                <div className="stat-details">
                  <span>Payées: {stats.factures.payees}</span>
                  <span>Impayées: {stats.factures.impayees}</span>
                </div>
                <div className="stat-amount">Payé: {formatPrix(stats.factures.montantRecouvre)}</div>
              </div>
            </div>
            ...
            <div className="stat-card livraisons-card">
              ...
              <div className="stat-content">
                ...
                <div className="stat-details">
                  <span>Domicile: {stats.livraisons.domicile}</span>
                  <span>Station: {stats.livraisons.station}</span>
                </div>
            ...
            <div className="chart-card">
              ...
              <div className="financial-stats">
                ...
                <div className="financial-item">
                  ...
                  <strong className="positive">{formatPrix(stats.factures.montantRecouvre)}</strong>
                </div>
                <div className="financial-item">
                ...
                <div className="delivery-options">
                  ...
                  <label className="delivery-option">
                    <input 
                      type="radio" 
                      value="domicile" 
                      checked={selectedTypeLivraison === 'domicile'} 
                  ...
                  <label className="delivery-option">
                    <input 
                      type="radio" 
                      value="station" 
                      checked={selectedTypeLivraison === 'station'} 
                      onChange={(e) => setSelectedTypeLivraison(e.target.value)} 
                    />
                    🏪 Retrait en station
                  </label>
            ...
            {factures.map(facture => (
              <div key={facture.id} className="facture-card">
                <div className="facture-header">
                  ...
                  <div className={`facture-status ${facture.statutPaiement === 'Payée' ? 'status-paid' : 'status-unpaid'}`}>
